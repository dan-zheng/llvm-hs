          ...
          then A.MDNode <$> decodeM n
            ...
            if v /= nullPtr
              then A.MDValue <$> decodeM v
                ...
                _ <- liftIO $ FFI.dumpMetadata md -- Dump the metadata for debugging purposes.
...
instance (MonadIO m, MonadAnyCont IO m, DecodeM m a (Ptr a')) => DecodeM m [a] (FFI.TupleArray a') where
  decodeM (FFI.TupleArray p)
    | p == nullPtr = pure []
    | otherwise = do
      -- size <- liftIO $ FFI.getMDNodeNumOperands (FFI.upCast p)
      -- traceMCallStack ("DecodeTupleArray length"  ++ show size)
      decodeArray FFI.getMDNodeNumOperands getOperand (FFI.upCast p)
    where
      getOperand md i = do
        -- traceMCallStack ("DecodeTupleArray getOperand call: " ++ show md ++ ", " ++ show i)
        (castPtr <$> FFI.getMDNodeOperand md i) :: IO (Ptr a')
